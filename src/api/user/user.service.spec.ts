...
describe('UserService', () => {
  ...
  it('should find one user by username', async () => {
    const result = await service.getUserByUsername('testUser0');
    expect(mockUserRepository.findOne).toHaveBeenCalledWith({where: {username: 'testUser0'}});
    expect(result).toEqual(mockUser);
  });
  ...
  it('should update an existing user, returning the entire new user object', async () => {
    ...
    const result = await service.updateUser(0, updateUser);
    expect(mockUserRepository.findOne).toHaveBeenCalledWith({where: {id: 0}});
